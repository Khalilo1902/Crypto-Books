...
import {displayAll, displayCoinById} from "../features/slice/CoinSlice.ts";
...
const TableComponent = () => {
    ...
    const displayCoinsId = useSelector((state:RootState)=>displayCoinById(state,coinId))
    console.log("DisplayCoin",displayCoinsId)
    console.log("CoinId",coinId)
    const cryptoItems: ICoinItem[] = useSelector(displayAll)
...
dispatch(setCoinId())
    ...
    ">
                ...
                <tbody>
                {
                    ...
                    <tr key={index} className="text-center text-base border-b border-gray-100
    ...
    ">
                <td className="py-4 flex items-center uppercase">
                    ...
                    <img className="w-[1.2rem] h-[1.2rem] mx-1.5" src={item.image} alt={item.name}/>
                ...
                <td className={
                    item.price_change_percentage_1h_in_currency > 0 ? "text-green py-4" : "text-red py-4"
                }>{item.price_change_percentage_1h_in_currency.toFixed(2)}</td>
                <td className={
                    item.price_change_percentage_24h_in_currency > 0 ? "text-green py-4" : "text-red py-4"
                }>{item.price_change_percentage_24h_in_currency.toFixed(2)}</td>
                <td className={
                    item.price_change_percentage_7d_in_currency > 0 ? "text-green py-4" : "text-red py-4"
                }>{item.price_change_percentage_7d_in_currency.toFixed(2)}</td>
                </tr>
                )) : (
    ...
    ">
                                <td className="py-4 flex items-center uppercase">
                                    ...
                                    <img className="w-[1.2rem] h-[1.2rem] mx-1.5" src={displayCoinsId.image} alt={displayCoinsId.name}/>
                                    <span>{displayCoinsId.symbol}</span>
                                </td>
                                <td className="py-4">{displayCoinsId.name}</td>
                                <td className="py-4">{
                                    new Intl.NumberFormat("en-US",{
                                        style:"currency",
                                        currency:"usd"
                                    }).format(displayCoinsId.current_price)
                                }</td>
                                <td className="py-4">{displayCoinsId.total_volume}</td>
                                <td className="py-4">{displayCoinsId.market_cap_change_percentage_24h}%</td>
                                <td className={
                                    displayCoinsId.price_change_percentage_1h_in_currency > 0 ? "text-green py-4" : "text-red py-4"
                                }>{displayCoinsId.price_change_percentage_1h_in_currency.toFixed(2)}</td>
                                <td className={
                                    displayCoinsId.price_change_percentage_24h_in_currency > 0 ? "text-green py-4" : "text-red py-4"
                                }>{displayCoinsId.price_change_percentage_24h_in_currency.toFixed(2)}</td>
                                <td className={
                                    displayCoinsId.price_change_percentage_7d_in_currency > 0 ? "text-green py-4" : "text-red py-4"
                                }>{displayCoinsId.price_change_percentage_7d_in_currency.toFixed(2)}</td>
                            </tr>
                        )
                }
                </tbody>
