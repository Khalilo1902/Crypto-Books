...
const TableComponent = ({
  ...
  const cryptoItems = useSelector(displayAllCoin);
  const getCoinById = useSelector((state: RootState) =>
    displayCoinById(state, coinId)
  );
  console.log("displayCoin: ", getCoinById);
  return (
    ...
    "
        ...
        <tbody>
          <>
            {coinId === "" ? (
              cryptoItems.slice(pre,next).map((item, index) => (
                <tr
                  key={index}
    ...
    "
                >
                  ...
                  <td className="py-4">{item.ath_change_percentage}%</td>
                  <td
                    className={
              ...
              >
                <td className="py-4 flex items-center uppercase">
                  ...
                  <img
                    className="w-[1.2rem] h-[1.2rem] mx-1.5"
                    src={getCoinById.image}
                    alt={getCoinById.name}
                  />
                  <span>{getCoinById.symbol}</span>
                </td>
                <td className="py-4">{getCoinById.name}</td>
                <td className="py-4">
                  {new Intl.NumberFormat("en-US", {
                    style: "currency",
                    currency: "usd",
                  }).format(getCoinById.current_price)}
                </td>
                <td className="py-4">{getCoinById.total_volume}</td>
                <td className="py-4">
                  {getCoinById.market_cap_change_percentage_24h}%
                </td>
                <td
                  className={
                    getCoinById.price_change_percentage_1h_in_currency > 0
                      ? "text-green py-4"
                      : "text-red py-4"
                  }
                >
                  {getCoinById.price_change_percentage_1h_in_currency}
                </td>
                <td
                  className={
                    getCoinById.price_change_percentage_24h_in_currency > 0
                      ? "text-green py-4"
                      : "text-red py-4"
                  }
                >
                  {getCoinById.price_change_percentage_24h_in_currency}
                </td>
                <td
                  className={
                    getCoinById.price_change_percentage_7d_in_currency > 0
                      ? "text-green py-4"
                      : "text-red py-4"
                  }
                >
                  {getCoinById.price_change_percentage_7d_in_currency}
                </td>
              </tr>
            )}
          </>
